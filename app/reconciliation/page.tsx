      ...
      <table className="w-full text-sm">
        <thead className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10">
          <tr className="text-zinc-500 text-[11px] uppercase tracking-wider">
            <th className="text-left px-3 py-2.5 font-medium w-[280px]">Item</th>
            <th className="text-center px-2 py-2.5 font-medium w-[80px]">Case size</th>
            <th className="text-center px-2 py-2.5 font-medium w-[80px]">A cases</th>
            <th className="text-center px-2 py-2.5 font-medium w-[80px]">A loose</th>
            <th className="text-center px-2 py-2.5 font-medium w-[80px]">B cases</th>
            <th className="text-center px-2 py-2.5 font-medium w-[80px]">B loose</th>
            <th className="text-left px-3 py-2.5 font-medium">Diff (live)</th>
            <th className="w-8" />
          </tr>
        </thead>
        <tbody>
    ...
    <Fragment>
      <tr className={cn("border-t border-zinc-200/50 dark:border-zinc-800/50 align-middle", rowTint)}>
        ...
        <td className="px-3 py-2 text-[11px] tabular-nums">
          <DiffPill label="A" side={rd.A} touched={rd.A.userTouched || rd.caseSizeChanged} />
          <DiffPill label="B" side={rd.B} touched={rd.B.userTouched || rd.caseSizeChanged} />
        </td>
        <td className="px-2 py-2">
          {myTouched && (
...
function DiffPill({ label, side, touched }: {
  label: "A" | "B";
  side: RowDiffType["A"];
  touched: boolean;
}) {
  if (!touched) {
    return <div className="flex items-center gap-1 text-zinc-400"><span className="text-[10px] font-medium">{label}</span><span>—</span></div>;
  }
  if (!side.valid) {
    return <div className="flex items-center gap-1 text-rose-600 dark:text-rose-400"><span className="text-[10px] font-medium">{label}</span><AlertCircle className="w-3 h-3" /> bad input</div>;
  }
  const colour = side.diff === 0
  ...
  return (
    <div className={cn("flex items-center gap-1", colour)}>
      ...
      <span className="text-[10px] text-zinc-500">{fmtN(side.appPieces)}→{fmtN(side.physPieces)}</span>
      <span className="font-semibold">{side.diff === 0 ? "✓" : (side.diff > 0 ? "+" : "") + fmtN(side.diff)}</span>
    </div>
  );
}
...
function GodownBlock({
  ...
  const diffColour = !touched ? "text-zinc-400"
    : !side.valid ? "text-rose-600 dark:text-rose-400"
    : side.diff === 0 ? "text-emerald-600 dark:text-emerald-400"
    : side.diff > 0 ? "text-cyan-600 dark:text-cyan-400"
    : "text-rose-600 dark:text-rose-400";
  return (
    <div className="bg-zinc-50 dark:bg-zinc-800/40 rounded-lg p-2">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={cn("text-[10px] tabular-nums font-medium", diffColour)}>
          {!touched ? "—" : !side.valid ? "bad" : side.diff === 0 ? "✓ match" : (side.diff > 0 ? "+" : "") + fmtN(side.diff)}
        </span>
      </div>
      <div className="space-y-1.5">
        <div className="grid grid-cols-[auto_1fr] gap-1.5 items-center">
          <span className="text-[10px] text-zinc-500 w-12">Cases</span>
          <ExprInput value={d_cases} placeholder={String(appCases)} onChange={onCases}
        ...
        <div className="grid grid-cols-[auto_1fr] gap-1.5 items-center">
          ...
          <ExprInput value={d_loose} placeholder={String(appLoose)} onChange={onLoose}
            tone={touched ? (side.valid ? "amber" : "rose") : "neutral"} ariaLabel={`${modelHint} loose`} />
        </div>
      </div>
      {touched && side.valid && (
        <div className="mt-1.5 text-[10px] text-zinc-500 tabular-nums text-center">
          app {fmtN(side.appPieces)} → phys {fmtN(side.physPieces)} pcs
        </div>
      )}
    </div>
  );

...
export function Providers({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: sub } = c.auth.onAuthStateChange(async (_event, session) => {
      ...
      // IMPORTANT: keep loading=true while we fetch the profile.
      // The redirect effect below early-returns when loading is true, which
      // prevents a race where pathname has just changed to "/" after sign-in
      // but profile hasn't loaded yet — without this, the user gets bounced
      // back to /login during that brief window.
      setLoading(true);
    ...
    c.auth
      ...
      .then(async ({ data: { session } }) => {
        ...
        // Same guard as above — keep loading=true through profile fetch.
        setLoading(true);
        const prof = await fetchProfile(session.user.id);
        if (cancelled) return;
        setProfile(prof);

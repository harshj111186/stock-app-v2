...
export default function Dashboard() {
  const [data, setData] = useState<{
    ...
    movedIds: Set<string>;
    loaded: boolean;
  }>({ items: [], txn: [], stockedIds: new Set(), movedIds: new Set(), loaded: false });

  useEffect(() => {
    (async () => {
      const c = sb();
      // 90-day cutoff for "dead stock" (items with stock that haven't sold recently).
      const ninety = new Date();
      ninety.setDate(ninety.getDate() - 90);
      const [{ data: items }, { data: stock }, { data: pricing }, { data: txn }, { data: recentSales }] = await Promise.all([
        ...
        c.from("transactions").select("item_id").eq("action", "Sale").gte("created_at", ninety.toISOString()),
      ]);
      ...
      // Items that have a real godown_stock row somewhere (any godown).
      // Distinguishes "ran out" (row exists, qty 0) from "never stocked" (no row).
      const stockedIds = new Set<string>((stock || []).map((s: any) => s.item_id as string));
      // Items with at least one Sale in the last 90 days.
      const movedIds = new Set<string>((recentSales || []).map((t: any) => t.item_id as string));
      setData({ items: combined, txn: (txn || []) as Txn[], stockedIds, movedIds, loaded: true });
    })();
  }, []);
  ...
  // Out of stock: has a godown_stock row but quantity is zero.
  // Previously this double-counted items that were catalogued but never stocked.
  const outOfStock = data.items.filter(i =>
    data.stockedIds.has(i.id) && (i.totalA + i.totalB === 0)
  ).length;
  // Never stocked: catalogued but no godown_stock row anywhere.
  const neverStocked = data.items.filter(i => !data.stockedIds.has(i.id)).length;
  // Dead stock: currently has stock but no Sale in the last 90 days.
  const deadStock = data.items.filter(i =>
    (i.totalA + i.totalB > 0) && !data.movedIds.has(i.id)
  ).length;
    ...
    <Shell title="Dashboard">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
        <Card title="Attention needed" icon={<BellRing className="w-4 h-4 text-amber-500" />}>
          ...
          <Attention label="Dead stock (no sale in 90 days)" count={deadStock} tone="neutral" />
          <Attention label="Never stocked" count={neverStocked} tone="neutral" />
        </Card>
